...
export const ReconstructionPage = () => {
  ...
  if (!user.profile.isadmin) {
    pageContents = useTracker(() =>
      Modules.find({ ageGroup: userAgeGroup }, {timePeriod: "Reconstruction Era"} ).fetch()
    );
  }
  else {
    pageContents = useTracker(() => 
      Modules.find( { timePeriod: "Reconstruction Era" } ).fetch()
    );
  }
  ...
  return (
    <div className="module-box">
      {user && user.profile.isadmin ? (
        <>
          <h1 className="module-header">Reconstruction Era</h1>
