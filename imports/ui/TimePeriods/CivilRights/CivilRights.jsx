...
export const CivilRightsPage = () => {
  ...
  if (!user.profile.isadmin) {
    pageContents = useTracker(() =>
      Modules.find({ ageGroup: userAgeGroup }, {timePeriod: "Civil Rights"} ).fetch()
    );
  }
  else {
    pageContents = useTracker(() => 
      Modules.find( { timePeriod: "Civil Rights" } ).fetch()
    );
  }
  ...
  return (
    <div className="module-box">
      {user && user.profile.isadmin ? (
        <>
          <h1 className="module-header">Civil Rights</h1>
