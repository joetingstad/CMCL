...
export const OOSTPage = () => {
  ...
  if (!user.profile.isadmin) {
    pageContents = useTracker(() =>
      Modules.find({ ageGroup: userAgeGroup }, {timePeriod: "Origins of the Slave Trade"} ).fetch()
    );
  }
  else {
    pageContents = useTracker(() => 
      Modules.find( { timePeriod: "Origins of the Slave Trade" } ).fetch()
    );
  }
  ...
  return (
    <div className="module-box">
      {user && user.profile.isadmin ? (
        <>
          <h1 className="module-header">Origins of the Slave Trade</h1>
