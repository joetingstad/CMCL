...
export const WW2Page = () => {
  ...
  if (!user.profile.isadmin) {
    pageContents = useTracker(() =>
      Modules.find({ ageGroup: userAgeGroup }, {timePeriod: "World War II"} ).fetch()
    );
  }
  else {
    pageContents = useTracker(() => 
      Modules.find( { timePeriod: "World War II" } ).fetch()
    );
  }
  ...
  return (
    <div className="module-box">
      {user && user.profile.isadmin ? (
        <>
          <h1 className="module-header">World War II</h1>
        ...
        <>
          <h1 className="module-header">World War IIr</h1>
          <br />
